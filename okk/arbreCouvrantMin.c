...
void prim(tasbinaire_t *a, int nbSommet, int sommet_source, tasbinaire_t * MST)
{
	...
	while(tout_visiter(visited, nbSommet) == 0)
	{
		...
		if(visited[actuel] == 0)
		{
			...
			/*Dans un graphe complet, chaque sommet a n-1 sommets adjacent
			tel que n est le nombre de sommet du graphe*/
			int position_sommet_adjacent[nbSommet-1];
			for(i =0; i<nombre_arete; i++)
			{
